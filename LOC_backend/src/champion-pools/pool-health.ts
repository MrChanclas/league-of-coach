import { MIN_CHAMPIONS_PER_ROLE, type PoolSlotKey } from './pool-roles';
...
export function computePoolHealth(
  entries: EnrichedPoolEntry[],
  slots: PoolSlotKey[],
): PoolHealth {
  // "Cubierta" means the slot reached the minimum to count as a real pool
  // (MIN_CHAMPIONS_PER_ROLE), not just "has one champion in it" — a slot with
  // 1-4 champions is still being built out. Only the account's own slots
  // (main lines + FILL) count, not all five roles.
  const countBySlot = new Map<string, number>();
  for (const entry of entries) {
    countBySlot.set(entry.role, (countBySlot.get(entry.role) ?? 0) + 1);
  }
  const rolesCovered = slots.filter(
    (slot) => (countBySlot.get(slot) ?? 0) >= MIN_CHAMPIONS_PER_ROLE,
  ).length;
  const coverage: PoolHealthNote = {
    label: `${rolesCovered} de ${slots.length} ${slots.length === 1 ? 'línea cubierta' : 'líneas cubiertas'}`,
  };
  ...
  if (enoughDataToScore) {
    const coverageScore = (rolesCovered / slots.length) * 100;
